...
fn main() {
    ...
    let wavf = wav::WavReader::new("wav/volume_up.wav");
    let samples = wavf.get_num_samples();
    let wav_in = graph.add_node(wavf);

    let comp = graph.add_node(dynamics::Compressor::new(-6.0, 0.5, 0.0, 1));
    let out = graph.add_node(wav::WavWriter::new("wav/test_compressor.wav", 1));
    graph.add_edge(wav_in, 0, comp, 0).unwrap();
    graph.add_edge(comp, 0, out, 0).unwrap();

    let lim = graph.add_node(dynamics::Limiter::new(-6.0, 0.0, 1));
    let out = graph.add_node(wav::WavWriter::new("wav/test_limiter.wav", 1));
    graph.add_edge(wav_in, 0, lim, 0).unwrap();
    graph.add_edge(lim, 0, out, 0).unwrap();

    let gate = graph.add_node(dynamics::NoiseGate::new(-6.0, -9.0, 0.0, 1));
    let out = graph.add_node(wav::WavWriter::new("wav/test_noise_gate.wav", 1));
